...
class CircuitEnv():
    ...
    def get_energy(self, thetas=None):
        ...
        shot_noise = 0#vc.get_shot_noise(self.weights, self.n_shots)        
    ...
    def global_roto(self, which_angles=[]):
        ...
        arguments = (self.hamiltonian, self.weights,param_circuit, self.num_qubits, self.energy_shift,self.n_shots)
    ...
    def adam_spsa_3(self, which_angles = [] ):
        ...
        def cost1(x):
            ...
            n_shots = int(self.n_shots/100), phys_noise = self.phys_noise,
                      which_angles=[])
        ...
        def cost2(x):
            ...
            weights = self.weights, circuit = qulacs_circuit, 
            n_qubits = self.num_qubits, energy_shift = self.energy_shift, 
            n_shots = int(self.n_shots/10), phys_noise = self.phys_noise,
                      which_angles=[])
        ...
        result_spsa = vc.min_spsa3_v2( fun1 = cost1, fun2 = cost2, fun3 = cost3,
                                   x0 = x0, **self.maxfevs,
                                 **self.options)
        ...
        return thetas, result_spsa["nfev"], result_spsa["x"]
    
    def adam_spsa_1(self, which_angles = [] ):
        ...
        result_spsa = vc.min_spsa_v2(cost,
                                   x0, **self.maxfev,
                                 **self.options)
    ...
    def spsa_optim(self,which_angles=[]):
        state = self.state.clone()
        thetas = state[:, self.num_qubits+3:]
        rot_pos = (state[:,self.num_qubits: self.num_qubits+3] == 1).nonzero( as_tuple = True )
        angles = thetas[rot_pos]
        ...
        def func(x):
            return vc.get_energy_qulacs(x,observable=self.hamiltonian,
                                           ...
                                           n_shots=int(self.n_shots/100),
                                           noise_value=self.noise_value,
                                           M=self.M,
                                           which_angles=which_angles)
        ...
        if list(which_angles):
            result_min_qulacs=vc.minimize_spsa(fun=func,x0=x0,maxfev=maxfev,maxiter=maxiter)
    ...
    def spsa_optim_3stage(self,which_angles=[]):
        options = {'a': 0.4835, 'alpha': 0.2026,'c': 0.0622, 'gamma': 0.0443}
        ...
        qulacs_inst = vc.Parametric_Circuit(n_qubits=self.num_qubits,noise_model=self.noise_model,noise_value=self.noise_value)
        qulacs_circuit = qulacs_inst.construct_ansatz(state)
        x0 = np.asarray(angles.cpu().detach())
        ...
        result_spsa = vc.minimize_spsa(fun=cost_, x0=x0, maxfev=7150, maxiter=7150,**options)
        ...
        def cost_(x):
            return vc.get_energy_qulacs(x,observable=self.hamiltonian,
                                           ...
                                           n_qubits=self.num_qubits, 
                                           energy_shift=self.energy_shift, 
                                           n_shots=int(self.n_shots/10),
                                           noise_value=self.noise_value,
                                           M=self.M,
                                           which_angles=which_angles)
        
        result_spsa = vc.minimize_spsa(fun=cost_, x0=x0, maxfev=2145, maxiter=2145,**options)
        ...
        result_spsa = vc.minimize_spsa(fun=cost_, x0=x0, maxfev=715, maxiter=715,**options)
